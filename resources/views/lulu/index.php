<!DOCTYPE HTML>
<!--[if IE 7 ]>    <html lang="en-gb" class="isie ie7 oldie no-js"> <![endif]-->
<!--[if IE 8 ]>    <html lang="en-gb" class="isie ie8 oldie no-js"> <![endif]-->
<!--[if IE 9 ]>    <html lang="en-gb" class="isie ie9 no-js"> <![endif]-->
<!--[if (gt IE 9)|!(IE)]><!--> 
<html lang="en-gb" class="no-js" ng-app="todoApp"> <!--<![endif]-->
<head>
<meta http-equiv="content-Type" content="text/html; charset=utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1"> 
<title>.:: Lulu | Meu diario ::.</title>
<meta name="description" content="">
<meta name="author" content="">
<link rel="shortcut icon" href="favicon.ico" type="image/x-icon" />
<link href="lulu/style.css" rel="stylesheet" type="text/css">
<link href="lulu/css/shortcodes.css" rel="stylesheet" type="text/css">
<link href="lulu/css/responsive.css" rel="stylesheet" type="text/css">
<link href="lulu/css/font-awesome.min.css" rel="stylesheet" type="text/css">
<link rel='stylesheet' id='layerslider-css' href="lulu/css/layerslider.css" type='text/css' media='all' />
<!--prettyPhoto-->
<link href="lulu/css/prettyPhoto.css" rel="stylesheet" type="text/css" media="all" /> 
<!-- Latest compiled and minified CSS -->
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">

<!--[if IE 7]>
<link href="lulu/css/font-awesome-ie7.css" rel="stylesheet" type="text/css">
<![endif]-->
<!--[if lt IE 9]>
<script src="http://html5shim.googlecode.com/svn/trunk/html5.js"></script>
<![endif]-->
<!--Fonts-->
<link href='http://fonts.googleapis.com/css?family=Lato:300,400,700,300italic,400italic,700italic' rel='stylesheet' type='text/css'>
<link href='http://fonts.googleapis.com/css?family=Open+Sans:300italic,400italic,600italic,700italic,800italic,400,300,600,700,800' rel='stylesheet' type='text/css'>
<link href='http://fonts.googleapis.com/css?family=Bubblegum+Sans' rel='stylesheet' type='text/css'>
<!--jquery-->
<script src="lulu/js/modernizr-2.6.2.min.js"></script>

</head>
<body class="main">
	<!--wrapper starts-->
    <div class="wrapper">
        <!--header starts-->
        <header>
            <div class="container">
                <div class="logo">
                    <a href="index.html" title="Lulu"></a>
                </div>
                
            </div>
            <!--menu-container starts-->
            <div id="menu-container">
                <div class="container">
                    <!--nav starts-->
                    <nav id="main-menu">
                    	<div class="dt-menu-toggle" id="dt-menu-toggle">Menu<span class="dt-menu-toggle-icon"></span></div>
                        <ul id="menu-main-menu" class="menu">
                            <li class="current_page_item menu-item-simple-parent menu-item-depth-0 red"> <a href="index.html"> Home </a> 
                                <ul class="sub-menu">
                                    <li> <a href="#"> Submenu Level I </a> </li>
                                    <li> <a href="#"> Submenu Level I </a>  </li>
                                    <li> <a href="#"> Submenu Level I </a> </li>
                                    <li> <a href="#"> Submenu Level I </a> </li>
                                    <li> <a href="#"> Submenu Level I </a> 
                                        <ul class="sub-menu">
                                            <li> <a href="#"> Submenu Level II </a> </li>
                                            <li> <a href="#"> Submenu Level II </a> </li>
                                            <li> <a href="#"> Submenu Level II </a>  </li>
                                        </ul>   
                                        <a class="dt-menu-expand">+</a>                             
                                    </li>
                                </ul>  
                                <a class="dt-menu-expand">+</a>                      
                            </li>
                            <li class="mustard"> <a href="about.html"> About us {{2+2}}</a> </li>
                            <li class="green"> <a href="our-staffs.html"> Our Staffs </a> </li>
                            <li class="yellow"> <a href="services.html"> Services </a> </li>
                            <li class="menu-item-simple-parent menu-item-depth-0 blue"><a href="portfolio-four-column.html" title="">Portfolio</a>
                                <ul class="sub-menu">
                                    <li><a href="portfolio-four-column.html">Portfolio Four Column</a></li>
                                    <li><a href="portfolio-three-column.html">Portfolio Three Column</a>
                                        <ul class="sub-menu">
                                            <li><a href="portfolio-three-column-with-sidebar.html">With Sidebar</a></li>
                                        </ul>
                                        <a class="dt-menu-expand">+</a>
                                    </li>
                                </ul>
                                <a class="dt-menu-expand">+</a>
                            </li>
                            
                            <li class="menu-item-megamenu-parent megamenu-4-columns-group menu-item-depth-0 steelblue"> <a href="#" title=""> Pages </a> 
                            
                                <div class="megamenu-child-container">
                                    <ul class="sub-menu">
                                        <!--<li> <a href="#">Useful Shortcodes</a>
                                            <ul class="sub-menu">
                                                <li> <a href="shortcodes.html">Typography </a> </li>
                                                <li><a href="buttons-lists.html" title="">Buttons &amp; Lists</a></li>
                                                <li><a href="columns.html" title="">Columns</a></li>
                                                <li><a href="fancy-boxes.html" title="">Fancy Boxes</a></li>
                                                <li><a href="icon-boxes.html" title="">Icon Boxes</a></li>
                                                <li><a href="pricing-tables.html" title="">Pricing Tables</a></li>
                                                <li><a href="progressbars.html" title="">Progress Bars</a></li>
                                                <li><a href="quotes.html" title="">Quotes</a></li>
                                    			<li><a href="tabs-toggles.html" title="">Tabs &amp; Toggles</a></li>
                                            </ul>
                                            <a class="dt-menu-expand">+</a> 
                                        </li>-->F
                                        <li class="menu-item menu-item-type-custom menu-item-object-custom menu-item-depth-1 menu-item-with-widget-area">
                                            <a href="#">Popular Products</a>
                                            <div class="menu-item-widget-area-container">
                                                <ul>
                                                    <li class="widget widget_popular_products">
                                                        <ul class="product_list_widget">
                                                            <li>
                                                                <a href="#" title="">
                                                                <img src="images/product_thumb1.jpg" alt="product"> Ellents Style Grade </a>
                                                                <span class="amount">$20.00</span>
                                                            </li>
                                                            <li>
                                                                <a href="#" title="">
                                                                <img src="images/product_thumb2.jpg" alt="product"> Ellents Style Grade </a>
                                                                <span class="amount">$20.00</span>
                                                            </li>
                                                            <li>
                                                                <a href="#" title="">
                                                                <img src="images/product_thumb3.jpg" alt="product"> Ellents Style Grade </a>
                                                                <span class="amount">$20.00</span>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                            </div>
                                        </li>
                                        <li class="menu-item menu-item-type-custom menu-item-object-custom menu-item-depth-1 menu-item-with-widget-area  fill-two-columns">
                                            <a href="#">Featured Blog</a>
                                            <div class="menu-item-widget-area-container">
                                            	<ul>
                                                	<li class="widget widget_recent_entries">
                                                    	<div class="entry-thumb">
                                                        	<a href="blog-detail.html"><img src="images/mega_menu_blog_img1.jpg" alt="" title=""></a>
                                                        </div>
                                                        <div class="entry-body">
                                                        	<p>Ut enim ad minima veniam, quis nostrum exercitationem ullam corporis suscipit.</p>
                                                        </div>
                                                        <div class="entry-details">
                                                        	<div class="entry-title">
                                                            	<h5><a href="blog-detail.html"> Create your Works </a></h5>
                                                            </div>
                                                            <div class="entry-metadata">
                                                            	<p class="author"><a href="#"> By Admin </a></p>
                                                                <span class=""> | </span>
                                                                <p class="comments"><a href="#"><span class="fa fa-comment"></span></a></p>
                                                                <p class="date"> 10 Aug </p>
                                                            </div>
                                                        </div>
                                                    </li>
                                                    <li class="widget widget_recent_entries">
                                                    	<div class="entry-thumb">
                                                        	<a href="blog-detail.html"><img src="images/mega_menu_blog_img2.jpg" alt="" title=""></a>
                                                        </div>
                                                        <div class="entry-body">
                                                        	<p>Minima ve nostrum exercita tionem ullam corporis suscipit laboriosam, nisi ut aliquid ex.</p>
                                                        </div>
                                                        <div class="entry-details">
                                                        	<div class="entry-title">
                                                            	<h5><a href="blog-detail.html"> Best of the Blogs </a></h5>
                                                            </div>
                                                            <div class="entry-metadata">
                                                            	<p class="author"><a href="#"> By Admin </a></p>
                                                                <span class=""> | </span>
                                                                <p class="comments"><a href="#"><span class="fa fa-comment"></span></a></p>
                                                                <p class="date"> 10 Aug </p>
                                                            </div>
                                                        </div>
                                                    </li>
                                                </ul>
                                                
                                            </div>
                                        </li>
                                    </ul>
                                </div>
                                <a class="dt-menu-expand">+</a>
                            </li>
                            <li class="menu-item-simple-parent menu-item-depth-0 lavender"><a href="blog.html" title="">Blog</a>
                                <ul class="sub-menu">
                                    <li><a href="blog-two-column.html">Blog Two Column</a>
                                    	<ul class="sub-menu">
                                            <li><a href="blog-two-column-with-sidebar.html">With Sidebar</a></li>
                                        </ul>
                                        <a class="dt-menu-expand">+</a>
                                    </li>
                                    <li><a href="blog.html">Blog One Column</a>
                                        <ul class="sub-menu">
                                            <li><a href="blog-with-sidebar.html">With Sidebar</a></li>
                                        </ul>
                                        <a class="dt-menu-expand">+</a>
                                    </li>
                                </ul>
                                <a class="dt-menu-expand">+</a>
                            </li>
                            <li class="purple"><a href="shop.html" title="">Shop</a></li>
                            <li class="pink"><a href="contact.html" title="">Contact us</a></li>
                        </ul>
                    </nav>
                    <!--nav ends-->

                    <ul class="dt-sc-social-icons">
                        <li><a href="#" title="Facebook" class="dt-sc-tooltip-top facebook"><span class="fa fa-facebook"></span></a></li>
                        <li><a href="#" title="Youtube" class="dt-sc-tooltip-top youtube"><span class="fa fa-youtube"></span></a></li>
                        <li><a href="#" title="Twitter" class="dt-sc-tooltip-top twitter"><span class="fa fa-twitter"></span></a></li>
                        <li><a href="#" title="Google Plus" class="dt-sc-tooltip-top gplus"><span class="fa fa-google-plus"></span></a></li>
                    </ul>
                </div>
            </div>
            <!--menu-container ends-->
        </header>
        <!--header ends-->
        <!--main starts-->
        <div id="main" ng-view>
        

        </div>
        <!--main ends-->
        
        <!--footer starts-->
        <footer>
            <!--footer-widgets-wrapper starts-->
            <div class="footer-widgets-wrapper">
                <!--container starts-->
                <div class="container">
                    
                    <div class="column dt-sc-one-fourth first">
                        <aside class="widget widget_text">
                            <h3 class="widgettitle red_sketch"> Sobre o Lulu </h3>
                            <p>Happy <a href=""><strong>Lulu</strong></a> monta suas historias e faz seu diario</p>
                            <ul>
                                <li> <a href=""> Ver historias </a> </li>
                                <li> <a href=""> nova historia </a> </li>
                     
                            </ul>
                        </aside>
                    </div>
                    <div class="column dt-sc-one-fourth">
                        <aside class="widget widget_recent_entries">
                            <h3 class="widgettitle green_sketch"> Meu amiguinhos </h3>
                            <ul>
                                <li>
                                    <a href="">
                                        <img src="images/blog_list1.jpg" alt="" title="">
                                    </a>    
                                    <h6><a href=""> Zezinho </a></h6>
                                    <span> escola </span>       
                                </li>
                                <li>
                                    <a href="">
                                        <img src="images/blog_list2.jpg" alt="" title="">
                                    </a>
                                    <h6><a href=""> Emilia </a></h6>
                                    <span> Sitio </span> 
                                </li>
                                <li>
                                    <a href="">
                                        <img src="images/blog_list3.jpg" alt="" title="">
                                    </a>
                                    <h6><a href=""> simon </a></h6>
                                    <span> Cheepmonkeys </span> 
                                </li>
                            </ul>
                        </aside>
                    </div>
                    <div class="column dt-sc-one-fourth">
                        <aside class="widget tweetbox">
                            <h3 class="widgettitle yellow_sketch"><a href="#"> Twitter Feeds </a></h3>
                            <div id="tweets_container"></div>
                        </aside>
                    </div>
                    <div class="column dt-sc-one-fourth">
                        <aside class="widget widget_text">
                        <h3 class="widgettitle steelblue_sketch">Contato</h3>
                            <div class="textwidget">
                                <p class="dt-sc-contact-info"><span class="fa fa-map-marker"></span> São Paulo - Sp <br> Brasil </p>
                                <p class="dt-sc-contact-info"><span class="fa fa-phone"></span> +55 (11) 233 - 3236 </p>
                                <p class="dt-sc-contact-info"><span class="fa fa-envelope"></span><a href="mailto:yourname@somemail.com"> lulu@lulu.com.br </a></p>
                            </div>
                        </aside>
                        <aside class="widget mailchimp">
                            <p> Somos sociais </p>
                            <form name="frmnewsletter" class="mailchimp-form" action="php/subscribe.php" method="post">
                                <p>
                                    <span class="fa fa-envelope-o"> </span>
                                    <input type="email" placeholder="Email" name="mc_email" required />	
                                </p>	
                                <input type="submit" value="Inscrever" class="button" name="btnsubscribe">
                            </form>
                            <div id="ajax_subscribe_msg"></div>
                        </aside>
                    </div>
                    
                </div>    
                <!--container ends-->
            </div>
            <!--footer-widgets-wrapper ends-->  
            <div class="copyright">
        		<div class="container">
                	<p class="copyright-info">© 2017 Lulu.</p>
        			<div class="footer-links">
                        <p>Saiba mais</p>
                        <ul class="dt-sc-social-icons">
                        	<li class="facebook"><a href="#"><img src="images/facebook.png" alt="" title=""></a></li>
 
                        </ul>
                    </div>
        		</div>
        	</div>  
        </footer>
        <!--footer ends-->
        
    </div>
    <!--wrapper ends-->
    <a href="" title="Go to Top" class="back-to-top">Topo ↑</a>
    <!--Java Scripts-->
    <script type="text/javascript" src="lulu/js/jquery.js"></script>
	<script type="text/javascript" src="lulu/js/jquery-migrate.min.js"></script>

   
	<script type="text/javascript" src="lulu/js/jquery.validate.min.js"></script>
	<script type="text/javascript" src="lulu/js/jquery-easing-1.3.js"></script>
    <script type="text/javascript" src="lulu/js/jquery.sticky.js"></script>
    <script type="text/javascript" src="lulu/js/jquery.nicescroll.min.js"></script>
    <script type="text/javascript" src="lulu/js/jquery.inview.js"></script>
    <script type="text/javascript" src="lulu/js/validation.js"></script>
    <script type="text/javascript" src="lulu/js/jquery.tipTip.minified.js"></script>
    <script type="text/javascript" src="lulu/js/jquery.bxslider.min.js"></script>       
    <script type="text/javascript" src="lulu/js/jquery.prettyPhoto.js"></script>  
    <script type="text/javascript" src="lulu/js/twitter/jquery.tweet.min.js"></script>
    <script type="text/javascript" src="lulu/js/jquery.parallax-1.1.3.js"></script>   
    <script type="text/javascript" src="lulu/js/shortcodes.js"></script>   
    <script type="text/javascript" src="lulu/js/custom.js"></script>
    
    <!-- Layer Slider --> 
    <script type="text/javascript" src="lulu/js/jquery-transit-modified.js"></script> 
    <script type="text/javascript" src="lulu/js/layerslider.kreaturamedia.jquery.js"></script> 
    <script type='text/javascript' src="lulu/js/greensock.js"></script> 
    <script type='text/javascript' src="lulu/js/layerslider.transitions.js"></script> 
    <!--<script type="text/javascript">var lsjQuery = jQuery;</script>--> 
    <script src="https://ajax.googleapis.com/ajax/libs/angularjs/1.6.4/angular.min.js"></script>
<script src="https://ajax.googleapis.com/ajax/libs/angularjs/1.6.4/angular-resource.min.js"></script>
<script src="https://ajax.googleapis.com/ajax/libs/angularjs/1.6.4/angular-route.min.js"></script>
<script type="text/javascript">
    angular.module('todoApp', ['ngRoute'])
    .config(function($routeProvider) {
          var resolveParams = {
            params: function () {
              return 'params';
            }
          };
         
          $routeProvider
            .when('/', {
              controller:'LuluController as Lulu',
              templateUrl:'views/home.html'
            })
            .when('/edit/:projectId', {
              controller:'EditProjectController as editProject',
              templateUrl:'detail.html',
              resolve: resolveParams
            })
            .when('/diario', {
              controller:'DiarioController as Diario',
              templateUrl:'views/diario.html',
              resolve: resolveParams
            })
            .when('/nova', {
              controller:'NovaHistoriaController as Nova',
              templateUrl:'views/nova.html',
              resolve: resolveParams
            })
            .when('/montar/:cenario', {
              controller:'MontarController as montar',
              templateUrl:'views/montar.html',
              resolve: resolveParams
            })
            .otherwise({
              redirectTo:'/'
            });
        })
      .controller('LuluController', function() {
        var Lulu = this;


    var lsjQuery = jQuery;


         //GALLERY SLIDESHOW...
    if(jQuery(".add-slider, .portfolio-slider, .about-slider").length) {
        jQuery('.add-slider, .portfolio-slider, .about-slider').bxSlider({
            auto:false, video:false, useCSS:false, pager:'true', autoHover:true, adaptiveHeight:true
        });
    }


     lsjQuery(document).ready(function() { if(typeof lsjQuery.fn.layerSlider == "undefined") { lsShowNotice('layerslider_1','jquery'); } else { lsjQuery("#layerslider_4").layerSlider({responsiveUnder: 1170, layersContainer: 1170, skinsPath: 'js/layerslider/skins/'}) } });
        
        Lulu.todos = [
          {text:'learn AngularJS', done:true},
          {text:'build an AngularJS app', done:false}];
     
        Lulu.addTodo = function() {
          Lulu.todos.push({text:Lulu.todoText, done:false});
          Lulu.todoText = '';
        };
     
        Lulu.remaining = function() {
          var count = 0;
          angular.forEach(Lulu.todos, function(todo) {
            count += todo.done ? 0 : 1;
          });
          return count;
        };
     
        Lulu.archive = function() {
          var oldTodos = Lulu.todos;
          Lulu.todos = [];
          angular.forEach(oldTodos, function(todo) {
            if (!todo.done) Lulu.todos.push(todo);
          });
        };
      })
      .controller('DiarioController', function() {
        alert("diario");
        var Diario = this;
       
      })
       .controller('NovaHistoriaController', function() {

        var Nova = this;
       
      });


</script>

    
</body>
</html>
